...
function plotFirstPoint(myChart, region, bounds) {
  const settings = {
    method: 'GET',
    headers: {
      Accept: 'application/json',
...
function addOptionMap (myChart,data, pointTitle, pointCname, bounds){
  console.log(data)
  ...
  if (size < 100) {
    option.series[0].blendMode = 'source-over';
  }
  return option;
}
function initNeighborhoodMap (myChart,data, pointTitle, pointCname, three_locations){
    console.log(data)
    const option = {
        title: {
            text: '北京地区Airbnb房源',
